...
def due(pdfpath):
    ...
    tree = ntree
    for element in tree.xpath('//*[not(*)][not(normalize-space())]'):
...
def main(pdfpath):
    ...
    FIND_FASE123T_PRIMA = re.compile(
        r"\]\s*prima\s*1\s*(?!\d+\s+)[^\s→]+(?:\s+(?!\d+\b)[^\s→]+)*\s*2\s+(?!\d+\b)[^\s→|T]+(?:\s+(?!\d+\b)[^\s→]+)*\s*$")  # SBAGLIATA
    FIND_FASE123T = re.compile(r"\]\s*1([\w\s]+)\s*2([\w\s]+)\s3\sT")
    FIND_FASE123T_OPZ2 = re.compile(
        r"\]\s*1\s*(?!\d+\s+)[^\s→]+(?:\s+(?!\d+\b)[^\s→]+)*\s*2\s+(?!\d+\b)[^\s→|T]+(?:\s+(?!\d+\b)[^\s→|T]+)*\s*$")
    ...
    FIND_FASE_PRIMA_12345T_OPZ2 = re.compile(
        r"\]\s*prima\s*1\s*([\w\s]+)\s*2([\w\s]+)\s*3([\w\s]+)\s*4\s*([\w\s][^T|5]+)$")
    FIND_FASE_12345T = re.compile(r"\]\s*1\s*([\w\s]+)\s*2([\w\s]+)\s*3([\w\s]+)\s*4([\w\s]+)\s*5\sT")
    FIND_FASE_12345T_OPZ_2 = re.compile(
        r"\]([\w\s][^prima]+)\s*1\s*([\w\s]+)\s*2([\w\s]+)\s*3([\w\s]+)\s*4([\w\s]+)\s*5\sT")
    ...
    FIND_FASE_PRIMA_123FRECCIA4T = re.compile(
        r"\]\s*prima\s*1\s*([\w\s]+)\s*2([\w\s]+)\s*3([\w\s][^4]+)\s*→\s*(T|4\s*T)")
    FIND_FASE_PRIMA_1FRECCIA234T = re.compile(
        r"\]\s*prima\s*1\s*([\w\s]+)\s*→\s*2([\w\s]+)\s*3([\w\s][^T]+)\s*(T|4\s*T)")
    FIND_FASE_PRIMA_12FRECCIA34T = re.compile(r"\]\s*prima\s*1\s*([\w\s]+)\s*2([\w\s]+)\s*→\s*3([\w\s]+)\s*(T|4\s*T)")
    FIND_FASE_PRIMA_FRECCIA1F2F4T = re.compile(
        r"\]\s*prima\s*1\s*([\w\s]+)\s*→\s*2([\w\s]+)\s*→\s*3([\w\s][^4]+)\s*→\s*T")
    ...
    FIND_FASE_RICAVATE_123456T = re.compile(
        r"\]\s*1\s*([\w\s]+)\s*2([\w\s]+)\s*3([\w\s]+)\s*4\s*([\w\s][^5]+)\s*5\s*([\w\s][^6]+)\s*→\s*(T)")
    ...
    df2 = pd.DataFrame([find_regex(find_prima, testoo),
                        ...
                        0,
                        find_regex(FIND_FASE_PRIMA_1FRECCIA234T, testoo, FIND_FASE_PRIMA_123FRECCIA4T,
    ...
    df2.rename(index={0: 'prima', 1: 'da', 2: 'sps.', 3: 'su', 4: 'as.', 5: 'ins.', 6: 'segue',
                      ...
                      19: 'fasericavata 1234T', 20: 'faseprima 1234T', 21: 'fasericavata T', 22: 'fasericavata 12345T',
                      23: 'fasericavata 123456T',
                      24: 'corpo minore', 25: 'fasericavataprima 1234T', 26: 'faseprima 123456T',
                      27: 'faseprima 12345678T'}, inplace=True)
    ...
    print(df)
    #print(df2)
    df2.to_csv('csv/sposipromessi/testonormalespt1c8.csv')
    df.to_csv('csv/sposipromessi/testofasispt1c8.csv')

print(main('spt1c8.pdf'))
